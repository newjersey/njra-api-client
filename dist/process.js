...
async function processBatch(apiKey, apiSecret, offset) {
    const response = await documented_1.request(apiKey, apiSecret, 'POST', `form/${FORM_ID}/pipeline`, {
        full_list: true,
        order_by: 'created_ts',
...
async function main() {
    const API_KEY = config_1.default.get('seamless.api.documented.key');
    const API_SECRET = config_1.default.get('seamless.api.documented.secret');
    let offset = START;
