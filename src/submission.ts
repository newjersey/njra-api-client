import { Application, ApplicationDatum } from './application';

type SubmissionDatum = string | Record<string, string>;
...
function toSubmissionDatum(applicationDatum: ApplicationDatum): SubmissionDatum {
...
export function getSubmissionData(application: Application): SubmissionData {
  const { application_data: applicationData } = application;
  const submission: Record<string, SubmissionDatum> = {};
  ...
  return (<unknown>submission) as SubmissionData;
}
